...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query(value = "SELECT DISTINCT groups.id as id , groups.name as name FROM groups INNER JOIN role ON groups.id = role.group_id INNER JOIN role_user ON role.id = role_user.role_id INNER JOIN user ON user.id = role_user.user_id WHERE user.user_name = ?1", nativeQuery = true)
